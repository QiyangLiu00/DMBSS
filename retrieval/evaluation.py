...
def evaluation(feature_path, lable_path, distance_func):
    ...
    if distance_func == 'euclidean_distance':
        ...
        # np.save(result_folder + '/distance.npy', {'Distance': distance})  # ——存入.npy文件
        # scio.savemat(result_folder+'/distance.mat', {'Distance': distance})   #——存入.mat文件

    elif distance_func == 'cos_sim':
        # a = np.dot(img_query_features, img_all_features.T)
        # b = (np.linalg.norm(img_query_features) * np.linalg.norm(img_all_features.T))
        # cos_sim = a / b
    ...
    for i in range(nQueries):
        ...
        # ==========================保存检索结果图像=================================
        # path = "../Data/UCM/UCM_test_list.txt"
        # img_path, targets = get_text_path(path)
        # retrieve_result = '../检索结果/UCM'
        # retrieve_path = os.path.join(retrieve_result, str(i))
        # os.makedirs(retrieve_path, exist_ok=True)
        # # 保存查询图像
        # queryimg = Image.open(img_path[i])
        # queryimg.save(os.path.join(retrieve_path, f'query_{current_labels[i]}.jpg'))
        # top = 10
        # for index in range(top):
        #     # 读取图像并另存
        #     image_temp_path = img_path[temp_idxs[index]]  # 获取第index张图像的路径
        #     target_temp = targets[temp_idxs[index]]
        #     img = Image.open(image_temp_path)
        #     output_path = os.path.join(retrieve_path, f'result_{index + 1}_{target_temp}.jpg')  # 设置另存的文件名，这里的序号是从1开始
    ...
    # np.save(result_folder + '/PR.npy', {'global_precision': global_precision, 'global_recall': global_recall})
    # # ——存入.npy文件
    # scio.savemat(result_folder + '/PR.mat', {'global_precision': global_precision, 'global_recall': global_recall})
    # # ——存入.mat文件
    #
    ...
    # ——存入.npy文件 np.save(result_folder + '/interpolated_PR.npy', {'interpolated_precision': interpolated_precision,
    # 'interpolated_recall': interpolated_recall}) ——存入.mat文件 scio.savemat(result_folder+'/interpolated_PR.mat',
...
if __name__ == "__main__":
    ...
    loadData_lable = np.load('./特征和标签/la.npy', allow_pickle=True)
    ...
    print('\n')
    print("high")
    evaluation(loadData_feature, loadData_lable, distance_func)
